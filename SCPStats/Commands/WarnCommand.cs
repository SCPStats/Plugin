    ...
    public class WarnCommand : ICommand, IUsageProvider
    {
        ...
        internal static bool ExecuteCustomDisplay(ArraySegment<string> arguments, ICommandSender sender, out string response, bool displayed, string usage, string success, string command, int type = 0)
        {
                ...
                switch (type)
                {
                    case 0:
                        foreach (var selectedPlayer in selectedPlayers)
                        {
                            ...
                            if (player?.UserId == null || player.IsServer || !player.IsReady || Helper.IsPlayerNPC(player)) continue;
            ...
            else
            {
                ...
                if (player?.UserId == null || player.IsServer || !player.IsReady || Helper.IsPlayerNPC(player))
                {
